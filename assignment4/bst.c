...
//we need stack.h because it has a predefined stack we can use for the iterator
#include "stack.h"
...
int bst_tracker(struct bst_node* root) {
	int size = 1;

	//count all the left leaves
	if (root->left != NULL) {
		size += bst_tracker(root->left);
	}

	//count all the right leaves
	if (root->right != NULL) {
		size += bst_tracker(root->right);
	}
...
int bst_height_tracker(struct bst_node* root) {


	if (root == NULL)
		return -1;

	//count nodes on left subtrees
		int left = bst_height_tracker(root->left);

	//count nodes on right subtrees

		int right = bst_height_tracker(root->right);

	//whichever one is higher is the height, and add 1 since # of recursive calls = height
	//if they are equal this works too since it's just an else statement that will always execute
	//if the first statement isn't true
...
int bst_height(struct bst* bst) {
	if (bst->root != NULL)
		return bst_height_tracker(bst->root);
}
...
int bst_path_exists(int sum, struct bst_node* root) {

		int subtract = sum - root->val;
		int check = 0;
		...
		if (root->left != NULL) {
			check = bst_path_exists(subtract, root->left);
		}if (root->right != NULL) {
			check = check || bst_path_exists(subtract, root->right);
		}
		return check;
	}
...
int bst_path_sum(int sum, struct bst* bst) {
	return bst_path_exists(sum, bst->root);
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	...
	//create a stack in the s_1 member variable of iterator struct
	iterator->s_1 = stack_create();

	struct bst_node* node = bst->root;
	while (node != NULL) {
		stack_push(iterator->s_1, node);
 ...
 */
int bst_iterator_has_next(struct bst_iterator* iterator) {
	if (stack_isempty(iterator->s_1) == 1)
		return 0;
	else 
		return 1;
}
...
int bst_iterator_next(struct bst_iterator* iterator) {
	//pointer to current node 
	struct bst_node* node = stack_pop(iterator->s_1);

	// value of current node we will be returning
	int value = node->val;

	//traverse right side of current node and add the values to the stack
	struct bst_node* leaf = node->right;
	while (leaf != NULL) {
		stack_push(iterator->s_1, leaf);
		leaf = leaf->left;
	}
	return value;
}
